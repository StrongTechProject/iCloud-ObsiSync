...
1.  **向导式完整初始化**:
    在终端输入 `obsis` 启动菜单，选择 **1. 快速开始**。此向导将引导您完成：
    *   设置 **Obsidian 笔记库路径 (Source)**。
    *   定义 **Git 仓库路径 (Destination)**。
    *   配置 GitHub 的 **SSH Key** (自动检测或生成)。
    *   **设置自动同步频率 (即“时光机”功能)**，真正实现“配置即遗忘”。

2.  **修改现有配置**:
    如果需要更改已设置的选项（例如同步频率或路径），请从主菜单选择 **3. 配置选项**。
