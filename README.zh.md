# iCloud-ObsiSync

[English README](README.md) | [详细文档](docs/zh/Doc.zh.md)

一个用于将 iCloud 或任意本地目录中的 Obsidian 笔记库自动同步到本地 Git 仓库并推送到 GitHub 的工具。

## ✨ 特性
- **自动同步**: 有条件地使用 `rsync` 将 Obsidian 笔记库镜像备份到本地 Git 仓库。如果笔记库本身就是 Git 仓库，则跳过 `rsync`。
- **版本控制**: 自动提交变更 (Commit) 并推送到远程 GitHub 仓库。
- **安全设计**: 敏感配置（路径、SSH Key）与代码分离，默认被 Git 忽略。
- **Cron 友好**: 专为定时任务设计，支持自动日志轮转和绝对路径处理。

## 📥 安装

**一键安装命令:**
```bash
/bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/StrongTechProject/iCloud-ObsiSync/main/src/install.sh)"
```

安装完成后，您可以在终端输入 `obsis` 直接启动管理菜单。

## 📋 环境准备
- **Git & GitHub 账号**: 你需要一个 GitHub 账号来存储你的笔记。
- **SSH Key 配置**: 本工具使用 SSH 协议进行安全传输。
  > 🆕 **新手必读**: 如果你没有配置过，请查看 [Git SSH 配置指南 (新手向)](docs/zh/Git_SSH_Config_Guide.zh.md) 了解详细步骤。

## 🚀 快速开始

1. **初始化配置**:
   运行安装脚本后，在终端输入 `obsis` 启动管理菜单，选择 `1. 快速开始`。
   或者直接运行 `./src/setup.sh` (首次安装后已设置好执行权限)。
   按照提示输入 Obsidian 笔记库路径和本地 Git 仓库路径。**配置向导现在允许你指定任意 Obsidian 笔记库路径 (iCloud 或本地)。如果 Obsidian 笔记库路径与本地 Git 仓库路径相同，脚本将直接对笔记库进行 Git 操作。**
   **在配置本地 Git 仓库时，如果检测到未配置远程仓库，脚本会引导您添加远程仓库地址。**

2. **手动运行**:
   ```bash
   obsis
   ```
   在菜单中选择相应的选项，或者直接运行同步脚本：
   ```bash
   ./src/sync_and_push.sh
   ```

3. **设置定时任务**:
   使用 `crontab -e` 添加定时任务，实现全自动备份。

详细说明请参考 [详细文档](docs/zh/Doc.zh.md)。
